...
void Lattice::initialise(string latType, int cnd, int latDim, bool pat, float fracH, float fracordH, float fracL, float fracordL, float fracLlin, VecF<int> patR, VecF<int> patL, string prefix,
...
void Lattice::threeandfourcoords() {
    ...
    fourcoordcount = nodes.n * fractionH;
    threecoordcount = nodes.n * (1-fractionH-fractionL);
    twocoordcount = nodes.n * fractionL;
    ...
    orderedtwocoord = twocoordcount*fractionorderedL;
    ...
    uniform_int_distribution<int> randNode(orderedfourcoord,nodes.n-1-orderedtwocoord);
    //////////////////////////////
...
//        cout << i << endl;
    ...
    i=nodes.n-1;
    while (i > nodes.n - twocoordcount - 1) {
//        cout << i << "               " << nodes.n - orderedtwocoord -1 << "         " << nodes.n - i - 1<< endl;
        if (i > nodes.n - orderedtwocoord-1){
            cout << "ORDERED 2" << endl;
            twocoord[nodes.n - i - 1] = i;
            ncoords[i] = int(nodeCnd-1);
            ncoordsoriginal[i] = int(nodeCnd-1);
            i-=1;
        }
...
//            cout << "DUMMY  :  " << dummy << endl;
            ...
            if (nodes.n - i - 1 < fourcoordcount){
                for (int j=0; j< nodes.n-i-1; j++){
                    cout << j << endl;
...
//            cout << "MADE IT TO 1122" << endl;

            if (compare){
                twocoord[nodes.n-i-1] = dummy;
                ncoords[i] = int(nodeCnd-1);
                ncoordsoriginal[i] = int(nodeCnd-1);
                i-=1;
            }
    ...
    for (i=orderedfourcoord;i<nodes.n-orderedtwocoord;i++){
        compare=true;
        for (int j=orderedfourcoord; j<fourcoordcount; j++){
            if (i == fourcoord[j]){
                compare = false;
            }
        }
        for (int j=orderedtwocoord; j<twocoordcount; j++){
            if (i == twocoord[j]){
                compare = false;
            }
        }
        if (compare){
            threecoord[runningcount] = i;
            ncoords[i] = int(nodeCnd);
            ncoordsoriginal[i] = int(nodeCnd);
            runningcount +=1;
        }
    }
}
...
void Lattice::initialiseProLattice() {
    ...
    if(!pattern) {
        for (int i = 0; i < fourcoordcount; i++) {
            nodes[fourcoord[i]].randomCnxs(nodeCnd + 1, mtGen);
        }
        for (int i = 0; i < threecoordcount; i++) {
            nodes[threecoord[i]].randomCnxs(nodeCnd, mtGen);
        }
        for (int i = 0; i < twocoordcount; i++) {
            nodes[twocoord[i]].randomCnxs(nodeCnd - 1, mtGen);
        }
    }
    else{
        if(randRL(mtGen)==0){
...
//    for (int i=20; i<24; ++i){
//        cout << i << "        " << nodes[i].crd <<endl;
        //        cout << i << "      " << nodes[i].getReciprocalCnxs(nodes).n << endl;
//    }
}
...
VecF<int> Lattice::generate(int maxIterations, double temperature, int mcsw) {
    ...
    cout << "Four" << "       " << "Three" << "       " << "Two" << "        " << nodes.n << endl;
    cout << fourcoordcount << "        " << threecoordcount << "        " << twocoordcount << "        " << nodes.n << endl;

    int energy = (threecoordcount*3 + fourcoordcount*4 + twocoordcount*2);
    int orderedfourcoord = fourcoordcount*fractionorderedH;
    int orderedtwocoord = twocoordcount*fractionorderedL;
...
//    cout << "SECONDARY ENERGY  :  " << energy << endl;
...
//    uniform_int_distribution<int> randNode(0,threecoordcount-1);
    ...
    for(;;) {
    //following line applies speedup if n+1 coordinate sites do not change on rotation (ie maximised)
//        int id=threecoord[randNode(mtGen)];
        int id = randNode(mtGen);
        int idcoord = randNode(mtGen);
        int coord;
        if (mcswitch == 1){
            coord = ncoordsoriginal[idcoord];
        }
        else{
            coord = ncoords[idcoord];
        }

//        int coord = ncoords[randNode(mtGen)]
        ...
        if (coord != 4) {
            if (!pattern) {
                nodes[id].randomCnxs(coord, mtGen);
            }
            else {
                if (randRL(mtGen) == 0) {
...
//        if (trialCnd > currCnd){
//           cout << currCnd << "-->" << trialCnd << "    " << energy << "    " << energy - 2*deltaCnd << endl;
//              }
        if(deltaCnd<0){
